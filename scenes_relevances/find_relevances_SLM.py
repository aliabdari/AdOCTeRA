...
def compute_similarity(list_sentences, slm_model):
    ...
    elif slm_model == 'distilroberta':
        model = SentenceTransformer('paraphrase-distilroberta-base-v2')
    elif slm_model == 'gte-large':
...
def process_sentences(root_descs_path, list_houses, slm_model):
    files = [x['json_file'] for x in list_houses]
    list_second_sents = []
    for f in files:
...
if __name__ == '__main__':
    slm_models = ['distilroberta', 'MiniLM', 'gte-large']
    selected_model_ind = 0
    print('Model = ', slm_models[selected_model_ind])
    file = open('../houses_data.pkl', 'rb')
    houses = pickle.load(file)
    root_descs_path = '../../FArMARe/descriptions'
    start_time = time.time()
    relations_norm = process_sentences(root_descs_path, houses, slm_models[selected_model_ind])
    with open(f'relevance_slm_normalized_{slm_models[selected_model_ind]}.pkl', 'wb') as pickle_file:
